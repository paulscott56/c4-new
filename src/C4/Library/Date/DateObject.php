...
use C4\Library\Cache\Storage\AdapterInterface as CacheAdapter;
...
/**
    ...
    /**
     * Internal mktime function used by C4\Library_Date.
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('C4\Library_DateObject_mkTime_' . $this->_offset . '_' . $year.$month.$day.'_'.$hour.$minute.$second . '_'.(int)$gmt, '-','_');
        ...
        if (isset(self::$_cache)) {
          if (self::$_cacheTags) {
            self::$_cache->setItem($id, $date, array('tags' => array('C4\Library_Date')));
          } else {
    ...
    /**
     * Internal mktime function used by C4\Library_Date for handling 64bit timestamps.
     *
    ...
    protected function date($format, $timestamp = null, $gmt = false)
    {
        ...
        $origstamp = $timestamp;
        if (isset(self::$_cache)) {
            $idstamp = strtr('C4\Library_DateObject_date_' . $this->_offset . '_'. $timestamp . '_'.(int)$gmt, '-','_');
        ...
        if (empty($gmt) and empty($jump)) {
            ...
            if (isset(self::$_cache)) {
              if (self::$_cacheTags) {
                self::$_cache->setItem($idstamp, $timestamp, array('tags' => array('C4\Library_Date')));
              } else {
    ...
    protected function getDateParts($timestamp = null, $fast = null)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('C4\Library_DateObject_getDateParts_' . $timestamp.'_'.(int)$fast, '-','_');
            if ($result = self::$_cache->getItem($id)) {
                return $result;
            }
        }
        ...
        if (isset(self::$_cache)) {
          if (self::$_cacheTags) {
            self::$_cache->setItem($id, $array, array('tags' => array('C4\Library_Date')));
          } else {
     ...
     * @return \C4\Library\Date\DateObject Provides fluent interface
     * @throws \C4\Library\Date\Exception
     */
